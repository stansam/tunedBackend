...
class AdminServiceCategoryDetailView(MethodView):
    ...
    def put(self, category_id: str):
        try:
            ...
            res = get_services().service_category.update_category(
                category_id, dto, actor_id=str(current_user.id)
            )
    ...
    def delete(self, category_id: str):
        try:
            get_services().service_category.delete_category(
                category_id, actor_id=str(current_user.id)
            )
...
class AdminServiceDetailView(MethodView):
    ...
    def put(self, service_id: str):
        try:
            ...
            dto = ServiceUpdateDTO(
                ...
                pricing_category_id=validated.get("pricing_category_id"),
                slug=validated.get("slug"),
                is_active=validated.get("is_active"),
                tags=validated.get("tags")
            )
            res = get_services().service.update_service(
                service_id, dto, actor_id=str(current_user.id)
            )
            return success_response(data=asdict(res))
        except ValidationError as err:
            return validation_error_response(err.messages)
    ...
    def delete(self, service_id: str):
        try:
            get_services().service.delete_service(
                service_id, actor_id=str(current_user.id)
            )
            return no_content_response()
        except Exception as exc:
